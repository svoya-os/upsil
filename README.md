...
> **Статус: v0.4 — ранняя версия.** Язык небольшой и честный: всё, что описано в
...
> то, чего не хватало ([что было неудобно и что исправлено](docs/dogfood.md)); 0.4 добавила
> решения с вероятностями. Чего пока нет — в разделе
> [«Чего нет в v0.4»](docs/spec.md#14-чего-нет-в-v04).
...
val kind = [m] => "О чём текст? {text}" -> choice(["код", "еда", "спорт"])   // решение: «код (97%)»

print(summary, strict, facts["язык"], kind.p)
...
- `-> choice([...])`, `-> yes`, `-> score(0..=3)` — не текст, а решение с вероятностями за
  один проход модели: программа действует сама, когда модель уверена, и зовёт человека, когда
  нет. `m.decide(текст, {...})` задаёт несколько таких вопросов сразу, а `llm.SystemOne`
  отправляет их на сервер с протоколом `/v1/systemone` (например, Jev).
...
- Джексон знает язык: с пакетом ставится навык [skills/upsil/SKILL.md](skills/upsil/SKILL.md),
  и на просьбу «напиши на упсиле…» он пишет код по правилам текущей версии.
...
**Решения.** Модели часто нужны не для текста, а для выбора: какой отдел ответит на
обращение, спам ли письмо, насколько клиент раздражён. Раньше для этого просили JSON и
надеялись, что модель ответит по форме; теперь есть `-> choice([...])`, `-> yes` и
`-> score(...)`. Модель не пишет ответ, а выбирает вариант за один проход по тексту, и UpsiL
возвращает вероятности, поэтому программа может сама решать, когда доверять модели, а когда
звать человека ([examples/triage.upl](examples/triage.upl)). Работает на локальном сервере
СОС; облачная модель Jev и совместимые с ней серверы — через `llm.SystemOne`. Подробности — в
[спецификации](docs/spec.md#96-решения-choice-yes-score) и [CHANGELOG.md](CHANGELOG.md).
