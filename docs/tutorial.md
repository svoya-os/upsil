...
Когда нужен не текст, а решение — какой отдел ответит, спам ли это, насколько клиент зол, —
спросите вероятности. `-> choice([...])` выбирает вариант, `-> yes` даёт вероятность «да»,
`-> score(...)` ставит оценку на шкале. Модель не пишет ответ, а выбирает вариант за один
проход по тексту, поэтому это быстро, а программа может действовать сама только тогда, когда
модель уверена:
...
llm m = llm.Model()
val mail = "Вы выиграли миллион! Перейдите по ссылке"
val spam = [m] => "Это спам? {mail}" -> yes
val kind = [m] => "Что это за письмо? {mail}" -> choice(["работа", "личное", "реклама"])
if spam > 0.9 {
    print("в спам")
} else if kind.p > 0.8 {
    print("в папку «{kind.value}»")
} else {
    print("разберу сам: {kind}")        // «реклама (64%)»
}
```

Несколько вопросов об одном тексте задаёт `m.decide`, а разбор обращений в поддержку целиком —
[examples/triage.upl](../examples/triage.upl) (раздел 9.6 спецификации).
