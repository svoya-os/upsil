from cortex.parser import Parser, FunctionDef, VarDecl, ReturnStmt, BinOp, Identifier, Number, FuncCall, Block, ImportStmt, ModelDef, StringLit, PromptCall, IfStmt, ForStmt, RangeExpr, ListLit, DictLit, BoolLit, LogicOp, GpuKernel
    ...
    def visit(self, node):
        if isinstance(node, Block):
            ...
            if mod.startswith("cortex."):
                mod = mod.replace("cortex.", "cortex.stdlib.")
            return f"import {mod}"
            ...
            if name.startswith("cortex."):
                name = name.replace("cortex.", "cortex.stdlib.")
            elif name == 'VectorStore':
                name = 'cortex.stdlib.rag.VectorStore'
            elif name == 'LLM':
                name = 'cortex.stdlib.llm.LLM'
            elif name == 'nn.Linear':
                name = 'nn.Linear'
...
def compile_cortex(code: str) -> str:
    tokens = lex(code)
