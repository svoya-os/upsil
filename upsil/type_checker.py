...
class upsilTypeError(Exception):
    pass
    ...
    def visit(self, node, env):
        if isinstance(node, Block):
            ...
            if node.var_type and expr_type and node.var_type != expr_type and node.var_type != "Any":
                raise upsilTypeError(f"Type mismatch: cannot assign {expr_type} to variable of type {node.var_type}")
            ...
            t = env.get(node.name)
            if not t:
                raise upsilTypeError(f"Undefined variable: {node.name}")
            return t
