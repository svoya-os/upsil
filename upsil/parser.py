...
class Parser:
    ...
    def parse_statement(self):
        ...
        elif curr.type == 'ID' and self.pos + 1 < len(self.tokens):
            # Check for reassignment: x = 5 or arr[0] = 5
            next_tok = self.tokens[self.pos + 1]
            if next_tok.type == 'ASSIGN':
                name = self.consume().value
                self.consume('ASSIGN')
                return AssignStmt(name, self.parse_expression())
            elif next_tok.type == 'PUNC' and next_tok.value == '[':
                # Could be array assignment arr[0] = 5
                # We need to look ahead further to see if there is an ASSIGN after ]
                # But a simpler way: parse as expression, and if it's followed by ASSIGN, convert it.
                pass
            
            # Let's handle it by parsing expression and checking if ASSIGN follows
            expr = self.parse_expression()
            if self.current() and self.current().type == 'ASSIGN':
                self.consume('ASSIGN')
                val_expr = self.parse_expression()
                if isinstance(expr, Identifier):
                    return AssignStmt(expr.name, val_expr)
                elif isinstance(expr, IndexExpr):
                    return IndexAssignStmt(expr.name, expr.index, val_expr)
                else:
    ...
    def parse_term(self):
        ...
        if curr.type == 'PUNC' and curr.value == '(':
            self.consume('PUNC', '(')
            expr = self.parse_expression()
            self.consume('PUNC', ')')
            return expr
